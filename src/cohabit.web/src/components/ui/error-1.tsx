  ...
  return (
    ...
    >
      <text
        ...
        className="fill-accent/20 stroke-accent font-black tracking-tighter"
...
function PillButton({ label, icon, href, onClick }: ErrorOneAction) {
  const cls =
    "h-10 rounded-full px-6 text-sm font-semibold rounded transition-opacity hover:opacity-90 mt-4 shadow-[inset_0_2px_4px_0_rgba(255,255,255,0.7),inset_0_-2px_4px_0_rgba(0,0,0,0.2)] sm:w-auto dark:shadow-[inset_0_-2px_4px_0_rgba(0,0,0,0.2),inset_0_2px_4px_0_rgba(255,255,255,0.5)] bg-accent text-accent-foreground hover:bg-accent/90";
  ...
  return (
    <main className="theme-injected bg-background text-foreground relative flex min-h-screen items-center justify-center px-6 py-16">
      <div
        ...
        style={{
          backgroundImage: `
        repeating-linear-gradient(0deg, transparent, transparent 19px, var(--accent) 19px, var(--accent) 20px, transparent 20px, transparent 39px, var(--accent) 39px, var(--accent) 40px),
        repeating-linear-gradient(90deg, transparent, transparent 19px, var(--accent) 19px, var(--accent) 20px, transparent 20px, transparent 39px, var(--accent) 39px, var(--accent) 40px),
        radial-gradient(circle at 20px 20px, var(--accent) 2px, transparent 2px),
        radial-gradient(circle at 40px 40px, var(--accent) 2px, transparent 2px)
      `,
      ...
      >
        ...
        <div className="flex flex-col items-center gap-2">
          <h1
            id="error-one-title"
            className="text-xl leading-snug font-bold tracking-tight text-accent sm:text-2xl"
          >
            {title}
          </h1>
          <p className="text-accent/80 mx-auto max-w-xs text-sm leading-relaxed sm:max-w-sm sm:text-base">
            {description}
          </p>
        </div>
        <PillButton {...action} />
