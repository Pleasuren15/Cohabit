...
export function MeetingCard({
    ...
    recording = true, aiNotes = true, participants, description
}: MeetingCardProps) {
    const [expanded, setExpanded] = useState(true);
    const [isRecording, setIsRecording] = useState(recording);
    const [isAiEnabled, setIsAiEnabled] = useState(aiNotes);
    const [going, setGoing] = useState<"Yes" | "No" | "Maybe" | null>(null);
                                    ...
                                    <div className="flex items-center gap-1.5 flex-wrap sm:flex-nowrap">
                                        {(["Yes", "No", "Maybe"] as const).map((opt) => (
                                            <button
                                                key={opt}
                                                type="button"
                                                aria-pressed={going === opt}
                                                onClick={() => setGoing(going === opt ? null : opt)}
                                                className={cn(
                                                    "px-2.5 sm:px-3 py-1 rounded-full border border-[#E5E5E5] dark:border-white/10 text-[11px] sm:text-[12px] font-medium text-[#374151] dark:text-gray-300 bg-white dark:bg-white/5 transition-colors",
                                                    going === opt
                                                        ? "border-[#22C55E] text-[#15803D] dark:text-green-400"
...
const Tag = ({ children, className = "" }: { children: React.ReactNode; className?: string }) => (
    <div className={cn("flex items-center gap-1 px-2 py-1 rounded-full border border-[#E5E5E5] dark:border-white/10 text-[12px] text-[#374151] dark:text-gray-300 bg-[#FAFAFA] dark:bg-white/5 whitespace-nowrap overflow-hidden", className)}>
        {children}
    </div>
);
...
const Toggle = ({
    ...
    return (
        <div
            ...
            className={`w-9 h-5 rounded-full px-1 flex items-center cursor-pointer transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-ring/50 focus-visible:outline-none ${
