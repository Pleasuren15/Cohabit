...
const Select33 = ({ onProvinceChange }: Select33Props) => {
  ...
  return (
    <div className="w-full max-w-xs space-y-2">
      ...
      <Select onValueChange={(value) => onProvinceChange?.(value)}>
        <SelectTrigger
          id={id}
          className="w-full rounded-xl border-zinc-200 bg-white px-2 shadow-xs transition-all hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950 dark:hover:bg-zinc-900"
        >
        ...
        >
          <SelectGroup>
            <SelectLabel className="px-2.5 text-xs text-zinc-500">
              South African Provinces
            </SelectLabel>
            {provinces.map((prov) => (
              <SelectItem key={prov.id} value={prov.id} className="rounded-lg">
      ...
      <p className="px-1 text-center text-[12px] text-zinc-500 dark:text-zinc-500">
