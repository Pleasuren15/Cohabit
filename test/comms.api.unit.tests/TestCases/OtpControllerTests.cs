...
public class OtpControllerTests
{
    ...
    {
        // Arrange
        var (controller, store, smsProvider, _, userId) = CreateSystemUnderTest();
        ...
        var sent = GetSentSms(smsProvider);
    ...
    {
        // Arrange
        var (controller, _, smsProvider, _, _) = CreateSystemUnderTest();

        // Act
        await controller.RequestOtp(new SendOtpRequest(OtpChannel.Email), CancellationToken.None);

        // Assert
        await smsProvider.DidNotReceiveWithAnyArgs().SendAsync(default!, default);
    }
    ...
    public async Task Given_CodeSentForSms_When_EmailRequestIsMade_Then_ChannelsAreCachedIndependently()
    {
        ...
        // Assert
        var smsCode = Regex.Match(GetSentSms(smsProvider).Body!, @"\d{6}").Value;
    ...
    public async Task Given_SecondSmsRequest_When_RequestOtpIsInvoked_Then_PreviousCodeIsReplaced()
    {
        ...
        // Assert
        var calls = smsProvider.ReceivedCalls()
            .Select(c => c.GetArguments().OfType<SmsMessage>().Single())
            .ToList();
    ...
    {
        // Arrange
        var (controller, store, smsProvider, _, _) = CreateSystemUnderTest(cellphone: null);
    ...
    public async Task Given_SmsProviderFails_When_SmsRequestIsMade_Then_ReturnsBadGatewayAndCodeIsNotCached()
    {
        // Arrange
        var (controller, store, smsProvider, _, userId) = CreateSystemUnderTest();
        smsProvider.SendAsync(Arg.Any<SmsMessage>(), Arg.Any<CancellationToken>())
    ...
    public async Task Given_SmsCodeSent_When_VerifyOtpIsInvokedWithMatchingCode_Then_ReturnsValidTrueAndConsumesCode()
    {
        ...
        var (controller, store, smsProvider, _, userId) = CreateSystemUnderTest();
        await controller.RequestOtp(new SendOtpRequest(OtpChannel.Sms), CancellationToken.None);
        var code = Regex.Match(GetSentSms(smsProvider).Body!, @"\d{6}").Value;
    ...
    private static (OtpController Controller, InMemoryOtpCodeStore Store, ISmsProvider SmsProvider, IResend Resend, Guid UserId)
            ...
            string? email = "thabo@example.com")
    {
        ...
        var smsProvider = Substitute.For<ISmsProvider>();
        ...
        var service = new OtpService(
            new SmsMessageDispatcher(smsProvider),
        ...
        return (controller, store, smsProvider, resend, resolvedUserId);
    }
    ...
    private static SmsMessage GetSentSms(ISmsProvider smsProvider)
    {
        return smsProvider.ReceivedCalls()
            .Single(c => c.GetMethodInfo().Name == nameof(ISmsProvider.SendAsync))
            .GetArguments()
            .OfType<SmsMessage>()
            .Single();
    }
